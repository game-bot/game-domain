...
export function createGameResourcesFromRewards(rewards: any[]) {
  ...
  for (const reward of rewards) {
    if (Array.isArray(reward.i)) {
      (reward.i as number[][]).forEach(item =>
        stats.inc(SmutstoneResources.items, item[item.length - 1])
      );
      ...
      (reward.cards as number[][]).forEach(item =>
        stats.inc(SmutstoneResources.cards, item[item.length - 1])
      );
