...
export default class DuelsClaimChestsJob extends SmutstoneJob {
  ...
  private async claimChests(
    player: Player,
    duelsData: PvpLoadApiData,
    userData: UserData
  ) {
    ...
    for (const chest of duelsData.chests) {
      ...
      const gameTime = userData.gameTime + (Date.now() - userData.updatedAt);
      if (endTime < gameTime) {
...
class ClaimChestTask extends SmutstoneTask {
  ...
  async innerExecute(player: Player, chestId: number) {
    const playerId = player.id;
    const response = await this.api.methodPvpChestClaim(player, {
      id: chestId,
      unlock: true
    });
    const errorDetails = this.createErrorDetails();
    ...
    const resources = createGameResourcesFromRewards([
      (data && data.reward) || {}
    ]).getData();
...
class ClaimStarChestTask extends SmutstoneApiTask<PvpClaimChestApiData> {
  ...
  protected createResources(data: PvpClaimChestApiData) {
    return createGameResourcesFromRewards([
      (data && data.reward) || {}
    ]).getData();
  }
}
