...
export type PvpChestApiData = {
  added: number;
  rarity: number;
  id: number;
  league: number;
};
export type PvpStarChestApiData = {
  stars: number;
  last: number;
  league: number;
};
...
export class PvpClaimChestApiDataParser extends EntityMapper<
...
export class PvpBattleFightApiDataMapper extends EntityMapper<
  PvpBattleFightApiData
> {
  constructor() {
    super(
      ["chest", "result", "starChest", "pointsGain"],
      fightBattleSchema.required()
    );
  }
}
