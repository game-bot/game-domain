...
import {
  PvpBattleFightApiDataMapper,
  PvpLoadApiDataParser,
  PvpClaimChestApiDataParser,
  PvpBattleFightApiData,
  PvpClaimChestApiData,
  PvpLoadApiData
} from "./data/api/pvp-data";
import { ApiEndpoints, ApiEndpointInfo } from "./data/endpoints";
import {
  CardsFightApiDataParser,
  CardsFightApiData
} from "./data/api/cards-fight-data";
import {
  LootboxOpenApiDataMapper,
  LootboxOpenApiData
} from "./data/api/toolbox-data";
...
export class SmutstoneApi extends BaseSmutstoneApi {
  constructor(
    ...
    super(repository, endpoints, defaultHeaders);
  }

  async methodPvpBattleFight(player: Player, args: { deck: number }) {
    return this.endpointCall<PvpBattleFightApiData>(
      ApiEndpoints.pvp_battle_fight,
      player,
      args
    );
  }
  ...
  async methodPvpStarchestClaim(player: Player) {
    return this.endpointCall<PvpClaimChestApiData>(
      ApiEndpoints.pvp_starchest_claim,
      player,
      {}
    );
  }

  async methodPvpChestClaim(
    player: Player,
    args: { id: number; unlock: boolean }
  ) {
    return this.endpointCall<PvpClaimChestApiData>(
      ApiEndpoints.pvp_chest_claim,
      player,
      args
    );
  }
  ...
  async methodLootboxOpen(player: Player, args: { type: number }) {
    return this.endpointCall<LootboxOpenApiData>(
      ApiEndpoints.lootbox_open,
      player,
      args
    );
  }

  async methodCardsBattleFight(
    player: Player,
    args: { location: number; mission: number; deck: number }
  ) {
    return this.endpointCall<CardsFightApiData>(
      ApiEndpoints.cards_battle_fight,
      player,
      args
    );
  }
