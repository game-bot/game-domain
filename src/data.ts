...
const DATA_GAME_JOB_MAP = DATA_GAMES_INFO_LIST.reduce<IDictionary<DataGameJobInfo>>((dic, game) => {
...
const GAME_JOBS_MAP = DATA_GAMES_INFO_LIST.reduce<IDictionary<GameJobInfo[]>>((dic, game) => {
    dic[game.id] = game.jobs;
    return dic;
}, {});

const GAMES_INFO_LIST = DATA_GAMES_INFO_LIST.map(item => pickDeep<GameInfo, GameInfo>(item, GameInfoFields))
const GAMES_INFO_MAP = GAMES_INFO_LIST.reduce<IDictionary<GameInfo>>((dic, info) => {
    dic[info.id] = info;
    return dic;
}, {});
